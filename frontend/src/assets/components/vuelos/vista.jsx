...
const TicketForm = () => {
  ...
  useEffect(() => {
    if (destination) {
      fetch(`https://aplicacion-tiquetes-back.vercel.app/api/vuelos/paises?query=${destination}`)
        .then(response => response.json())
        .then(data => {
          console.log(data); // Muestra los datos recibidos en la consola
...
const handleSaveTicket = async () => {
  if (origin && destination && date) {
    try {
      ...
      const response = await fetch('https://aplicacion-tiquetes-back.vercel.app/api/vuelos/boletos', {
        method: 'POST',
        headers: {
