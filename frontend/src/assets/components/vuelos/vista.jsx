...
const TicketForm = () => {
  ...
  useEffect(() => {
    if (origin) {
      fetch(`http://localhost:3000/vuelos/paises?query=${origin}`)
  ...
  useEffect(() => {
    if (destination) {
      fetch(`http://localhost:3000/vuelos/paises?query=${destination}`)
        .then(response => response.json())
        .then(data => {
          console.log(data); // Muestra los datos recibidos en la consola
...
const handleSaveTicket = async () => {
  if (origin && destination && date) {
    try {
      ...
      const response = await fetch('http://localhost:3000/vuelos/boletos', {
        method: 'POST',
        headers: {
